...
| 2019 | Chrome Web Content Swap | This chrome extension swaps out useless thumbnail and recommendations and change it to your selection queue. | Deprecated | ```Docker-Compose```, ```OPML```, ```Rancher```, ```RSS```, ```Ruby```, ```S3```, ```Slack```, ```Spreadsheet``` | <ul><li>[Github: Repository](https://github.com/a6b8/chrome-web-content-swap)</ul></li> |
